...
\end{equation}
when the terms are non-zero. Cancellation in \(N_\ast\) or a small \(D_\ast\)
can amplify rounded changes in the wave-speed estimate, and the amplified value
may then enter a discontinuous branch selection \citep{toro2009}.
...
Rusanov, and fp32/fp64. Exact-Riemann stopping tolerances, limiter choice, and
CPU/GPU reduction ordering remain concept-only. CPU architecture and
...
\(\mathbf{B}\), and the fluxes add magnetic-pressure, magnetic-tension, and
induction terms. The Riemann fan contains fast magnetosonic, Alfven, slow
magnetosonic, and contact/entropy families, not only acoustic waves and a
contact \citep{toro2009}. In an MHD CFL estimate, the fast magnetosonic speed
replaces the acoustic speed in Equation~\ref{eq:ch3-cfl-timestep}, while the
slow and Alfven speeds depend on field direction. The magnetic field must also
satisfy
\begin{equation}\label{eq:ch3-divB-zero}
  \nabla\cdot\mathbf{B}=0,
\end{equation}
at the continuous level. A finite-volume update does not preserve this
constraint automatically, so non-zero discrete divergence can create spurious
magnetic forces and distort wave speeds.
...
instead preserves a staggered-mesh discrete divergence constraint. These MHD
options only frame the later project: Report~1 evidence remains Euler-only,
with exact or high-resolution references and matched precision/device
comparisons. In ideal MHD, Roe-type and HLLD approximate Riemann solvers extend the HLL/HLLC family to the seven-wave fan; their selection forms part of the Report~2 numerical-method work. Ideal MHD will need separate tests of magnetic waves, energy
coupling, and divergence control.
% <<SECTION_6_END>>
