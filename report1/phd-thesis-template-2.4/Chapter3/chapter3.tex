...
used below \citep{toro2009,leveque_2002}. Equation~\ref{eq:fv-update} fixes the
...
while retaining second-order reconstruction in smooth monotone regions
\citep{vanleer_1979,toro2009}. Minbee is a diffusive TVD limiter, so small
...
\[
  ...
  \end{cases}
\]
The report's main validation flux is HLLC, which approximates this fan by left,
contact, and right waves with speeds \(S_L\), \(S_\ast\), and \(S_R\)
\citep{toro2009,toro_spruce_speares_1994}. In the implemented Davis estimate,
\[
...
This is the part of HLLC that restores the contact wave missing from a two-wave
HLL approximation \citep{toro_spruce_speares_1994,toro2009}. The exact choice of strict or non-strict
...
smear contacts and shear layers relative to HLLC
\citep{harten_lax_vanleer_1983,toro2009}.
% <<SECTION_3_END>>
...
constraint automatically, so divergence cleaning \citep{dedner_2002} or
constrained-transport \citep{evans_hawley_1988} choices become part of the
