// tests/unit/test_mhd_gpu_brio_wu.cpp
//
// CUDA-only end-to-end Brio-Wu agreement test for the GPU MHD solver.

#include "catch.hpp"

#ifdef HRSC_HAS_CUDA

#include "gpu/mhd_gpu_solver.hpp"
#include "mhd/mhd_solver.hpp"

#include <cstring>

using namespace hrsc;

namespace {

template <typename Real>
bool byte_equal(const Grid2D<Real, MhdNVars>& a,
                const Grid2D<Real, MhdNVars>& b) {
    return a.data.size() == b.data.size() &&
           std::memcmp(a.data.data(), b.data.data(),
                       a.data.size() * sizeof(Real)) == 0;
}

template <typename Real>
Grid2D<Real, MhdNVars> make_brio_wu_host(int nx, Real dx, Real xmin,
                                         Real gamma) {
    Grid2D<Real, MhdNVars> g(nx, 1);
    g.dx = dx;
    g.dy = dx;
    setup_brio_wu(g.view(), nx, dx, xmin, gamma, Real(0.5));
    return g;
}

template <typename Real>
Grid2D<Real, MhdNVars> snapshot_cpu_grid(MhdSolver<Real, HllFlux>& solver) {
    auto gv = solver.grid_view();
    Grid2D<Real, MhdNVars> out(gv.nx, gv.ny);
    out.dx = gv.dx;
    out.dy = gv.dy;
    auto ov = out.view();
    constexpr int ng = GridView<Real, MhdNVars>::ng;
    for (int j = -ng; j < gv.ny + ng; ++j) {
        for (int i = -ng; i < gv.nx + ng; ++i) {
            for (int v = 0; v < MhdNVars; ++v) {
                ov(i, j, v) = gv(i, j, v);
            }
        }
    }
    return out;
}

template <typename Real>
void require_brio_wu_gpu_matches_cpu() {
    constexpr int nx = 64;
    const Real xmin = Real(0);
    const Real dx = Real(1) / static_cast<Real>(nx);
    const Real gamma = Real(2);
    const Real cfl = Real(0.4);
    const TimeReal t_end = TimeReal(0.01);

    MhdSolver<Real, HllFlux> cpu(nx, dx, xmin, gamma, cfl, t_end,
                                 BoundaryType::Outflow);
    setup_brio_wu(cpu.grid_view(), nx, dx, xmin, gamma, Real(0.5));
    cpu.run();

    auto host = make_brio_wu_host(nx, dx, xmin, gamma);
    MhdGpuSolver<Real> gpu(host, xmin, Real(0), gamma, cfl, t_end, Real(0),
                           BoundaryType::Outflow, BoundaryType::Outflow);
    gpu.run();

    REQUIRE(gpu.step_count() == cpu.step_count());
    REQUIRE(gpu.current_time() == cpu.time());
    REQUIRE(byte_equal(gpu.download_host_grid(), snapshot_cpu_grid(cpu)));
}

} // namespace

TEST_CASE("MHD GPU Brio-Wu 1D matches CPU HLL end-to-end",
          "[gpu][mhd][brio-wu]") {
    require_brio_wu_gpu_matches_cpu<double>();
    require_brio_wu_gpu_matches_cpu<float>();
}

#endif // HRSC_HAS_CUDA
