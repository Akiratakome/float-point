...
TEST_CASE("LW Config 4 IC sample cells match published values", "[lw][lw4]") {
    ...
    REQUIRE(q1[PRHO] == eq(1.1));
    REQUIRE(q1[VX] == eq(0.0));
    REQUIRE(q1[VY] == eq(0.0));
    REQUIRE(q1[PRES] == eq(1.1));

    auto q2 = read_prim(0, 3);  // x<0.5, y>0.5
    REQUIRE(q2[PRHO] == eq(0.5065));
    REQUIRE(q2[VX] == eq(0.8939));
    REQUIRE(q2[VY] == eq(0.0));
    REQUIRE(q2[PRES] == eq(0.35));

    auto q3 = read_prim(0, 0);  // x<0.5, y<0.5
    REQUIRE(q3[PRHO] == eq(1.1));
    REQUIRE(q3[VX] == eq(0.8939));
    REQUIRE(q3[VY] == eq(0.8939));
    REQUIRE(q3[PRES] == eq(1.1));

    auto q4 = read_prim(3, 0);  // x>0.5, y<0.5
    REQUIRE(q4[PRHO] == eq(0.5065));
    REQUIRE(q4[VX] == eq(0.0));
    REQUIRE(q4[VY] == eq(0.8939));
    REQUIRE(q4[PRES] == eq(0.35));
}

TEST_CASE("LW Config 4 runs one step without NaN or Inf", "[lw][lw4]") {
    using Real = double;
    constexpr int nx = 16;
    constexpr int ny = 16;
    constexpr Real gamma = 1.4;
