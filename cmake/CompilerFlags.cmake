...
if(OPT_LEVEL)
    ...
    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
        if(OPT_LEVEL STREQUAL "O2")
            set(_hrsc_opt_flags /O2)
        elseif(OPT_LEVEL STREQUAL "O3")
            set(_hrsc_opt_flags /Ox)
        elseif(OPT_LEVEL STREQUAL "Ofast")
            set(_hrsc_opt_flags /Ox /fp:fast)
        endif()
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang|AppleClang")
        set(_hrsc_opt_flags "-${OPT_LEVEL}")
    else()
        message(FATAL_ERROR "OPT_LEVEL is not mapped for C++ compiler '${CMAKE_CXX_COMPILER_ID}'")
    endif()
    add_compile_options(${_hrsc_opt_flags})
    string(REPLACE ";" " " _hrsc_opt_flags_msg "${_hrsc_opt_flags}")
    message(STATUS "HRSC optimisation axis: ${_hrsc_opt_flags_msg}")
endif()

if(FAST_MATH)
    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
        set(_hrsc_fast_math_flags /fp:fast)
    elseif(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang|AppleClang")
        set(_hrsc_fast_math_flags -ffast-math)
    else()
        message(FATAL_ERROR "FAST_MATH is not mapped for C++ compiler '${CMAKE_CXX_COMPILER_ID}'")
    endif()
    add_compile_options(${_hrsc_fast_math_flags})
    string(REPLACE ";" " " _hrsc_fast_math_flags_msg "${_hrsc_fast_math_flags}")
    message(STATUS "HRSC fast-math axis: ${_hrsc_fast_math_flags_msg}")
endif()
