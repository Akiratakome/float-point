...
def _save_report_figure(fig: plt.Figure, out_path: Path, dpi: int = 180) -> None:
    """Keep raster previews and LaTeX's preferred PDF output in sync."""
...
def plot_fmd_bar(rows: list[dict[str, float | str]], out_path: Path) -> None:
    ...
    ax.legend()
    fig.tight_layout()
    _save_report_figure(fig, out_path, dpi=180)
    plt.close(fig)
...
def plot_region_losos(rows: list[dict[str, float | str | int]], out_path: Path) -> None:
    ...
    for ax, solver in zip(axes, ("hllc", "rusanov")):
        ...
        ax.axhline(s_req, color="black", linewidth=1.4, linestyle="--", label="s_req")
        ax.set_xticks(x, [short[r] for r in regions])
        ax.set_title(f"{solver.upper()}, p32 rho", fontsize=18)
        ax.set_xlabel("gradient region", fontsize=16)
        ax.tick_params(axis="both", labelsize=15)
        ax.grid(True, axis="y", alpha=0.3)
    axes[0].set_ylabel("LoSoS significant digits", fontsize=16)
    ...
    _save_report_figure(fig, out_path, dpi=300)
    plt.close(fig)
...
def plot_losos_quantiles(rows: list[dict[str, float | str | int]], out_path: Path) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(10.4, 5.8), sharey=True)
    ...
    for ax, solver in zip(axes, ("hllc", "rusanov")):
        ...
        ax.set_title(solver.upper(), fontsize=18)
        ax.set_xticks(bits, labels)
        ax.set_xlabel("virtual precision", fontsize=16)
        ax.tick_params(axis="both", labelsize=15)
        ax.grid(True, alpha=0.3)
    axes[0].set_ylabel("LoSoS rho significant digits", fontsize=16)
