...
|---|---:|---|---|
| `experiments/<weekN>/cpu_gpu_midtime/summary.md` | P0 (planned) | Intermediate-time CPU vs GPU snapshots on Sod, LW3, and (if cheap) LW12. Run each case under `solver=hllc`, `STRICT_IEEE=ON`, with 3-5 mid-evolution checkpoint outputs (e.g., t = T/4, T/2, 3T/4 plus final). For each checkpoint and each precision (fp32, fp64), compute L1, Linf, and ULP_max between the matched CPU and GPU binaries on the conservative state. Use the same matched-binary toolchain principle as the existing device summaries (one binary per within-case CPU-vs-GPU comparison). Summary must report whether mid-time drift is zero (bit-identical), small, or growing. | §5.5 (bounds the "final-time only" caveat with one optional table row); §6.3 (whether device drift accumulates in time). |
| `experiments/<weekN>/variation_fp32/summary.md` | P0 (planned) | fp32 × compiler-flag mini-matrix on Sod (and optionally Toro3). CPU only. Axes: O2 vs O3, O2 vs Ofast±fast-math. Same harness as `report1_variation/`. For each combination, report L1, Linf, and ULP_max between the chosen flag and the O2 baseline, on the conservative state at final time. Summary must state explicitly that this row is fp32 and that all prior `report1_variation` rows are CPU double. | §5.6a (one fp32 row in the variation table); §6.3 (comparison of "compiler effect vs precision effect"). |
| `experiments/<weekN>/variation_limiter/summary.md` | P1 (planned, upgrade if available) | Limiter variation on Sod and (optionally) LW3-N200. CPU only, fp64 baseline + one fp32 row per limiter. Axes: minmod vs van Leer (vs any other limiter actually implemented in the source). Same harness as `report1_variation/`. For each limiter pair, report L1, Linf, and ULP_max on the conservative state at final time. Summary must say whether limiter swaps produce non-trivial final-time drift or remain near zero. | §3.5 (upgrades the "limiter branch" axis from concept-level to quantitative); §5.6a (additional row if it materially changes drift). |
