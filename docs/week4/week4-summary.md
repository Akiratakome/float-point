...
- Production: 200²×30 run on Cambridge LSC `lovelace` with single-node process parallelism, /dev/urandom seed CSV shards (no flock). The planned SLURM-array path was not used for the final production run; the output layout was kept compatible with the analyzer.
