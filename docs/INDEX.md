...
| `experiments/week15/mhd_temporal_divergence/` | Week-15 fp32-vs-fp64 HLL temporal density-drift evidence: Brio-Wu 1D (15 paired samples, fixed fit window `[0.01, 0.1]`, L1 lambda 30.6153) and Orszag-Tang 2D (25 paired samples, fixed fit window `[0.1, 0.5]`, L1 lambda 0.0293431, Linf lambda -0.0422334). The planned OT>Brio-Wu contrast is not observed. The passing gate establishes technical completeness, finite/nonnegative drift samples, and positive OT L1 lambda only; these are bounded Lyapunov-like engineering fits without an independently gated fit-quality or formal physical-exponent claim. Unified `summary.{csv,json,md}` + `figures/temporal_divergence.png`; run configs/logs/metadata retained, transient grids removed. |
