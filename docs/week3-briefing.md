...
**Date:** 2026-04-09
**Prior:** Week 1 (core infrastructure) + Week 2 (1D MUSCL-Hancock + HLLC)
**Branch:** main (`8540139`)
...
| Main program | `src/main.cpp` | Config-driven, text output |
...
### 1.3 Known Deviations from Original Plan
...
```bash
# Build (default, non-strict inequality)
cmake -B build -S . -G Ninja && cmake --build build

# Build (strict inequality variant)
...
Based on the design spec's "Out of Scope (Week 2)" section and the Week 2 report's "next work" notes, Week 3 should cover:
...
|----------|------|-----------|
| 1 | Y-direction functions (5.3) | Foundation for all 2D work |
| 2 | 2D Euler Solver (5.4) | Enables 2D test cases |
| 3 | Additional limiters (5.2) | Quick to add, useful for comparison |
| 4 | Binary IO (5.5) | Needed once 2D data gets large |
| 5 | Exact Riemann C++ (5.1) | Reference solver, already have Python version |
| 6 | Error norms C++ (5.6) | Convergence study support |
| 7 | Grid convergence (5.7) | Validation milestone |
