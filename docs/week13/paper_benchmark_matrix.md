...
Week 13 HLLD-vs-HLL solver-comparison summary, a Week 13 Orszag-Tang
`256^2` paper-style figure packet, and a Week 13 Kelvin-Helmholtz `256^2`
paper-style figure packet. The full Orszag-Tang and Kelvin-Helmholtz `512^2`
self-reference gates remain planned follow-up evidence, not completed
...
| Orszag-Tang 2D | HLL | Toth 2000, DOI `10.1006/jcph.2000.6519`; Orszag-Tang vortex benchmark context | `tests/cases/orszag_tang_2d/orszag_tang.cfg`, `t=0.5` | density/pressure/divB maps | 256² paper-style figure packet exists; full 512² self-reference gate is pending due local runtime | validates benchmark morphology and finite diagnostics; 512-grid self-reference is secondary and must not be claimed until rerun |
| Kelvin-Helmholtz 2D | HLL | Frank et al. 1996, arXiv `astro-ph/9510115`; Lecoanet et al. 2015, arXiv `1509.03630` limitation | `tests/cases/kelvin_helmholtz_2d/kh.cfg`, `t=1.0` | density, magnetic field magnitude, divB maps | 256² paper-style figure packet exists; full 512² self-reference gate pending | bounded morphology/stability evidence; avoid overclaiming convergence because KH can be ill-posed |
