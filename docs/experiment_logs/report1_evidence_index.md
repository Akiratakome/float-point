...
| Section 4 Validation: 1D Toro, 2D LW3, CPU vs GPU, float vs double, convergence | 1D Toro3/Toro5 HLLC strict CPU-to-GPU comparison | `experiments/week7/report1_validation_1d_device/cpu_vs_gpu_toro3_toro5_hllc_strict.md` | new / ready | Report 1 evidence completion |
...
| CPU/GPU strict rows for the selected Report 1 validation cases currently show zero final-state drift. | This supports reproducibility for Sod, Toro3, Toro5, and LW3, but does not demonstrate hardware-driven divergence or cover the full Toro/LW catalogue. | Treat as a validation pass for the selected cases; use branch-rule, compiler, fast-math, or longer-window runs for divergence evidence. |
